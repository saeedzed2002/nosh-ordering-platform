...
def list_menu_items(
    ...
    availability_by_item_id = {item.id: AvailabilityState.AVAILABLE for item in items}
    if location_slug is not None:
        location = session.scalar(
            select(Location).where(
                Location.slug == location_slug, Location.is_published.is_(True)
            )
        )
        if location is None:
            raise HTTPException(
        ...
        availability_rows = session.execute(
            select(MenuItemAvailability.menu_item_id, MenuItemAvailability.state).where(
                MenuItemAvailability.location_id == location.id
            )
        ).all()
        availability_by_item_id = {
            menu_item_id: AvailabilityState(state)
            for menu_item_id, state in availability_rows
        }
...
def read_home_content(session: SessionDep) -> list[HomeContentResponse]:
    content_blocks = session.scalars(
        ...
        .options(joinedload(HomeContent.media))
